...
public class EntriesController : ControllerBase
{
    ...
    {
        return await _service.GetByCategoryAsync(categoryId, User.GetUserId());
    }

    [HttpGet("{id:guid}")]
    public async Task<ActionResult<EntryDto>> GetById(Guid id)
    {
        var entry = await _service.GetByIdAsync(id, User.GetUserId());
    ...
    {
        var entry = await _service.CreateAsync(dto, User.GetUserId());
    ...
    {
        var entry = await _service.UpdateAsync(id, dto, User.GetUserId());
        return entry is null ? NotFound() : Ok(entry);
    }
